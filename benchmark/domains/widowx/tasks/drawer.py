...
class Drawer(BaseTask):
    def __init__(self):
        ...
        path = os.path.join(ASSERT_PATH, "Drawer", "model.xml")
        self._drawer = XMLObject(path, scale=0.11)
        self._arena.attach(self._drawer)
    ...
    def initialize_episode(self, physics, random_state):
        ...
        pos = random_state.uniform(low=[0.2, 0.225, 0.035], high=[0.25, 0.275, 0.035])
        euler = random_state.uniform(
            (0, 0, -np.pi / 2 - np.pi / 32), (0, 0, -np.pi / 2 + np.pi / 32)
        )
        self._drawer.set_pose(physics, position=pos, quaternion=tr.euler_to_quat(euler))
        pos[-1] = 0.02
        offset = np.full(fill_value=0.01, shape=(3,))
        pos = distributions.Uniform(pos - offset, pos + offset)
