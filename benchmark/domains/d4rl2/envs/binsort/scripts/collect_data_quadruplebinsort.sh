NUM_OBJ=4
ENV=BinSort-v0
OUTPUT=/nfs/kun2/users/asap7772/bridge_data_exps/sim_data/minibullet/0802_addednoise_4obj
...
for ((i=0; i<${NUM_OBJ} ; i++)); do
    for ((j=0; j<${NUM_OBJ} ; j++)); do
        for ((k=0; k<${NUM_OBJ} ; k++)); do
            for ((l=0; l<${NUM_OBJ} ; l++)); do
                for policy in "${policies[@]}"; do
                    if [ $i -lt $j ] && [ $j -lt $k ] && [ $k -lt $l ]; then
                        COMMAND="python scripts/scripted_collect.py -e ${ENV} -pl ${policy} -n ${amount} -t ${timesteps} \
                        -d ${OUTPUT}_${policy}/task_${i}_${j} -a ${accept}  --noise 0.1 --specific_task_id 1 --desired_task_id $i $j $k $l --save-all &"
