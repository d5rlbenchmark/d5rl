...
from d4rl2.envs.widowx.roboverse.bullet.serializable import Serializable
...
class Widow250Env(gym.Env, Serializable):
    ...
    def _load_meshes(self):
        ...
        self.robot_id = objects.widow250(scale=0.1)
        ...
        for object_name, object_position in zip(self.object_names,
                                                object_positions):
            print('in widowx env: ', object_name, object_position)
    ...
    def reset(self):
        ...
        self._load_meshes()
        bullet.reset_robot(self.robot_id, self.reset_joint_indices,
    ...
    def step(self, action):
        ...
        ee_pos, ee_quat = bullet.get_link_state(self.robot_id,
        ...
        target_ee_pos = np.clip(target_ee_pos, self.ee_pos_low,
                                self.ee_pos_high)
        target_gripper_state = np.clip(target_gripper_state,
                                       GRIPPER_LIMITS_LOW, GRIPPER_LIMITS_HIGH)

        bullet.apply_action_ik(target_ee_pos,
                               target_ee_quat,
        ...
        if self.use_neutral_action and neutral_action > 0.5:
            if self.neutral_gripper_open:
                bullet.move_to_neutral(self.robot_id, self.reset_joint_indices,
                                       RESET_JOINT_VALUES)
            else:
                bullet.move_to_neutral(self.robot_id, self.reset_joint_indices,
                                       RESET_JOINT_VALUES_GRIPPER_CLOSED)
    ...
    def get_observation(self):
        ...
        if self.target_object is not None:
            # if-condition needed since some drawer environments have no objects.
    ...
    def render_obs(self):
        img, depth, segmentation = bullet.render(self.observation_img_dim,
