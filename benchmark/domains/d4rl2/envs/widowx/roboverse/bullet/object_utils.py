...
from d4rl2.envs.widowx.roboverse.bullet.button_utils import *
from d4rl2.envs.widowx.roboverse.bullet.drawer_utils import *

from .control import get_link_state, get_object_position
...
def check_in_container(object_name,
    ...
    if object_height < place_success_height_threshold:
        if container_half_extents is None:
            object_container_distance = np.linalg.norm(object_xy -
                                                       container_center_xy)
            if object_container_distance < place_success_radius_threshold:
                success = True
        else:
            if in_rectangle(object_xy, container_center_xy,
                            container_half_extents):
                success = True

    return success


def check_under_height_threshold(object_name, object_id_map,
                                 place_success_height_threshold):
    object_pos, _ = get_object_position(object_id_map[object_name])
    object_height = object_pos[2]
...
def check_grasp(
    ...
    if object_height > grasp_success_height_threshold:
        ee_pos, _ = get_link_state(robot_id, end_effector_index)
        object_gripper_distance = np.linalg.norm(object_pos - ee_pos)
...
def generate_object_positions_single(small_object_position_low,
                                     small_object_position_high,
                                     large_object_position_low,
                                     large_object_position_high,
                                     min_distance_large_obj=0.04):
    ...
    while not valid:
        ...
        valid = np.linalg.norm(small_object_positions[0] -
                               large_object_position) > min_distance_large_obj
        if i > max_attempts:
...
def generate_object_positions_v2(small_object_position_low,
                                 small_object_position_high,
                                 large_object_position_low,
                                 large_object_position_high,
    ...
    while not valid:
        ...
        valid_1 = np.linalg.norm(
            small_object_positions[0] -
            small_object_positions[1]) > min_distance_small_obj
        valid_2 = np.linalg.norm(
            small_object_positions[0] -
            large_object_position) > min_distance_large_obj
        valid_3 = np.linalg.norm(
            small_object_positions[1] -
            large_object_position) > min_distance_large_obj
...
def generate_object_positions_reverse(small_object_position_low,
                                      small_object_position_high,
                                      large_object_position_low,
                                      large_object_position_high,
                                      target_object,
    ...
    while not valid:
        ...
        if container_half_extents is None:
            small_object_position_target = np.random.uniform(
                low=large_object_position - place_success_radius_threshold,
                high=large_object_position + place_success_radius_threshold)
            small_object_position_target[-1] = large_object_position[-1]
        else:
            container_half_extents_z = np.concatenate(
                [container_half_extents, np.array([0])], axis=0)
            small_object_position_target = np.random.uniform(
                low=large_object_position - container_half_extents_z,
                high=large_object_position + container_half_extents_z)

        object_container_distance = np.linalg.norm(
            small_object_position_target[:2] - large_object_position[:2])

        valid_1 = np.linalg.norm(small_object_position - large_object_position
                                 ) > min_distance_large_obj
        if container_half_extents is None:
            valid_2 = object_container_distance < place_success_radius_threshold
        else:
            valid_2 = in_rectangle(small_object_position_target[:2],
                                   large_object_position[:2],
                                   container_half_extents)
...
def generate_object_positions(object_position_low,
                              object_position_high,
                              num_objects,
                              min_distance=0.05,
                              current_positions=None):
    if current_positions is None:
        object_positions = np.random.uniform(low=object_position_low,
                                             high=object_position_high)
    ...
    while object_positions.shape[0] < num_objects:
        ...
        object_position_candidate = np.reshape(object_position_candidate,
                                               (1, 3))
        min_distance_so_far = []
...
def load_object(object_name,
    ...
    if object_name in shapenet_obj_path_map.keys():
        return load_shapenet_object(object_name,
                                    object_position,
...
def load_shapenet_object(object_name,
                         object_position,
                         object_quat=(1, -1, 0, 0),
                         scale=1.0):
...
BULLET_OBJECT_SPECS = dict(
    ...
    drawer=dict(
        fileName=os.path.join(BASE_ASSET_PATH,
                              'drawer/drawer_with_tray_inside.urdf'),
        basePosition=(.7, 0.2, -.35),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.1,
    ),
    drawer_no_handle=dict(
        fileName=os.path.join(BASE_ASSET_PATH, 'drawer/drawer_no_handle.urdf'),
        basePosition=(.7, 0.2, -.35),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.1,
    ),
    ...
    low_tray=dict(
        fileName=os.path.join(BASE_ASSET_PATH, 'tray/low_tray.urdf'),
        basePosition=(.7, 0.2, -.35),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.25,
    ),
    low_tray_big=dict(
        fileName=os.path.join(BASE_ASSET_PATH, 'tray/low_tray_big.urdf'),
        basePosition=(.7, 0.2, -.35),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.25,
    ),
    open_box=dict(
        fileName=os.path.join(BASE_ASSET_PATH,
                              'box_open_top/box_open_top.urdf'),
        basePosition=(.7, 0.2, -.35),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.25,
    ),
    ...
    cube=dict(
        fileName=os.path.join(BASE_ASSET_PATH, 'cube/cube.urdf'),
        basePosition=(.7, 0.2, -.35),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.05,
    ),
    spam=dict(
        fileName=os.path.join(BASE_ASSET_PATH, 'spam/spam.urdf'),
        basePosition=(.7, 0.2, -.35),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.25,
    ),
    pan_tefal=dict(
        fileName=os.path.join(BULLET3_ASSET_PATH, 'dinnerware/pan_tefal.urdf'),
        basePosition=(.65, 0.3, -.3),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.8,
        useFixedBase=1,
    ...
    table_top=dict(
        fileName=os.path.join(BULLET3_ASSET_PATH, 'table/table2.urdf'),
        basePosition=(.65, 0.3, -.3),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.8,
        useFixedBase=1,
    ...
    checkerboard_table=dict(
        fileName=os.path.join(BULLET3_ASSET_PATH,
    ...
    torus=dict(
        fileName=os.path.join(BULLET3_ASSET_PATH, 'torus/torus.urdf'),
        basePosition=(.65, 0.3, -.3),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.8,
        useFixedBase=1,
    ...
    cube_concave=dict(
        fileName=os.path.join(BULLET3_ASSET_PATH, 'cube_concave.urdf'),
        basePosition=(.65, 0.3, -.3),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.8,
        useFixedBase=1,
    ...
    plate=dict(
        fileName=os.path.join(BULLET3_ASSET_PATH, 'dinnerware/plate.urdf'),
        basePosition=(.65, 0.3, -.3),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.8,
        useFixedBase=1,
    ...
    husky=dict(
        fileName=os.path.join(BULLET3_ASSET_PATH, 'husky/husky.urdf'),
        basePosition=(.65, 0.3, -.3),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.8,
        useFixedBase=1,
    ...
    marble_cube=dict(
        fileName=os.path.join(BULLET3_ASSET_PATH, 'marble_cube.urdf'),
        basePosition=(.65, 0.3, -.3),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.8,
        useFixedBase=1,
    ...
    button=dict(
        fileName=os.path.join(BASE_ASSET_PATH, 'button/button.urdf'),
        basePosition=(.7, 0.2, -.35),
        baseOrientation=(0, 0, 0.707107, 0.707107),
        globalScaling=0.1,
    ),
