...
import d4rl2.envs.widowx.roboverse.bullet.control as control
...
def get_drawer_base_joint(drawer):
    joint_names = [
        control.get_joint_info(drawer, j, 'jointName')
...
def get_drawer_handle_link(drawer):
    link_names = [
        bullet.get_joint_info(drawer, j, 'linkName')
...
def get_drawer_pos(drawer):
    drawer_pos, _ = bullet.get_link_state(drawer,
                                          get_drawer_base_joint(drawer))
    return np.array(drawer_pos)


def get_drawer_handle_pos(drawer):
    handle_pos, _ = bullet.get_link_state(drawer,
                                          get_drawer_handle_link(drawer))
    return np.array(handle_pos)


def get_drawer_opened_percentage(left_opening, min_x_pos, max_x_pos,
                                 drawer_x_pos):
    if left_opening:
...
def slide_drawer(drawer, direction, half_slide=False):
    ...
    p.setJointMotorControl2(drawer,
