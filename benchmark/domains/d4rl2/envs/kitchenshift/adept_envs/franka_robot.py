...
class Robot(BaseRobot):
    ...
    def _read_specs_from_config(self, robot_configs):
        ...
        for i in range(self.n_dofs):
            self.robot_mode[i] = read_config_from_node(root, "qpos" + str(i),
                                                       "mode", int)
    ...
    def get_obs(self,
                env,
                robot_noise_ratio=1,
                object_noise_ratio=1,
                sim_mimic_hardware=True):
        if self.is_hardware:
            raise NotImplementedError()
            ...
            if not env.initializing:
                qp += (
                    robot_noise_ratio * self.robot_pos_noise_amp[:self.n_jnt] *
                    env.np_random.uniform(low=-1.0, high=1.0, size=self.n_jnt))
                qv += (
                    robot_noise_ratio * self.robot_vel_noise_amp[:self.n_jnt] *
                    env.np_random.uniform(low=-1.0, high=1.0, size=self.n_jnt))
                if self.has_obj:
                    qp_obj += (robot_noise_ratio *
                               self.robot_pos_noise_amp[-self.n_obj:] *
                               env.np_random.uniform(
                                   low=-1.0, high=1.0, size=self.n_obj))
                    qv_obj += (robot_noise_ratio *
                               self.robot_vel_noise_amp[-self.n_obj:] *
                               env.np_random.uniform(
                                   low=-1.0, high=1.0, size=self.n_obj))

        # cache observations
        obs = observation(time=self.time,
                          qpos_robot=qp,
                          qvel_robot=qv,
                          qpos_object=qp_obj,
                          qvel_object=qv_obj)
    ...
    def step(
        ...
        if enforce_limits:
            if mode == 'velact':
                ...
                ctrl_feasible = self.ctrl_velocity_limits_velact(
                    ctrl_desired,
                    step_duration)  # this converts ctrl_desired in vel to pos
            elif mode == 'posact':
                # enforce velocity limits
                ctrl_feasible = self.ctrl_velocity_limits_posact(
                    ctrl_desired, step_duration)
            else:
                raise ValueError
        ...
        if self.is_hardware and (not sim_override):
            ...
            env.do_simulation(
                ctrl_feasible,
                int(step_duration /
                    env.sim.model.opt.timestep))  # render is folded in here

        # Update current robot state on the overlay
        if self.overlay:
            env.sim.data.qpos[self.n_jnt:2 *
                              self.n_jnt] = env.desired_pose.copy()
            env.sim.forward()
    ...
    def reset(self,
        ...
        if self.is_hardware:
            ...
            if self.has_obj:
                env.sim.data.qpos[-self.n_obj:] = reset_pose[-self.
                                                             n_obj:].copy()
        ...
        if self.overlay:
            env.sim.data.qpos[self.n_jnt:2 *
                              self.n_jnt] = env.desired_pose[:self.n_jnt].copy(
                              )
            env.sim.forward()
    ...
    def ctrl_velocity_limits_posact(self, ctrl_position, step_duration):
        ...
        ctrl_desired_vel = (ctrl_position -
                            last_obs.qpos_robot[:self.n_jnt]) / step_duration
        ...
        ctrl_feasible_position = (last_obs.qpos_robot[:self.n_jnt] +
                                  ctrl_feasible_vel * step_duration)
        return ctrl_feasible_position
