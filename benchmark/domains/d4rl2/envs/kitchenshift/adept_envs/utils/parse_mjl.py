# from https://github.com/vikashplus/puppet/blob/master/vive/source/parse_mjl.py
...
def parse_mjl_logs(read_filename, skipamount):
    ...
    dat = np.asarray(
        struct.unpack(str(num_floats) + 'f', fileContent[28 + name_len:]))
    ...
    mocap_pos = dat[nq + nv + nu + 1:nq + nv + nu + 3 * nmocap +
                    1, :].T[::skipamount, :]
    mocap_quat = dat[nq + nv + nu + 3 * nmocap + 1:nq + nv + nu + 7 * nmocap +
                     1, :].T[::skipamount, :]
    sensordata = dat[nq + nv + nu + 7 * nmocap + 1:nq + nv + nu + 7 * nmocap +
                     nsensordata + 1, :].T[::skipamount, :]
    userdata = dat[nq + nv + nu + 7 * nmocap + nsensordata +
                   1:, :].T[::skipamount, :]
