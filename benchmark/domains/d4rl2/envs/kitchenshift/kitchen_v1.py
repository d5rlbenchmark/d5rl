...
class Kitchen_v1(gym.Env):
    ...
    def __init__(
        ...
        camera_keys = ['camera_{}'.format(idx) for idx in camera_ids]
        if self.use_wrist_cam:
            camera_keys += ['camera_gripper']
...
# cast to float32
    ...
    def _get_reward_n_score(self, obs_dict):
        ...
        for element in self.tasks_to_complete:
            element_idx = OBS_ELEMENT_INDICES[element]
            element_pos = next_obj_obs[..., element_idx -
                                       self.N_DOF_ROBOT].copy()
            element_goal = self.goal[element_idx].copy()
    ...
    def step(self, action):
        ...
        reward, reward_dict = self._get_reward_n_score(obs_dict)
        #        obs = {k: obs_dict[k] for k in self.observation_space.keys()}
        done = False
        #        env_info = {k: obs_dict[k] for k in obs_dict.keys() if 'rgb' not in k}

        env_info.update({"reward " + key:float(val) for key, val in reward_dict.items()}) ###===### ###---###
