...
set_dataset_path(
    os.environ.get(
        "FINTUNE_DATASET_DIR", os.path.expanduser("~/.finetuning_benchmark/datasets")
    )
)
...
class FinetuneEnv(gym.Wrapper):
    """
    Base class for finetuning envs, taken largely from D4RL.
    ...
    def __init__(
        ...
        super(FinetuneEnv, self).__init__(env)
    ...
    def get_dataset(self, h5path=None):
        ...
        if "observations" not in data_dict:
            ...
            for k in all_keys:
                if "observations/" in k:
                    obs_key, obs_name = k.split("/")
                    assert len(k.split("/")) == 2
                    data_dict[obs_key][obs_name] = data_dict[k]
                    del data_dict[k]
        ...
        N_samples = data_dict["actions"].shape[0]
        # TODO: Add checks for obs.
        """
