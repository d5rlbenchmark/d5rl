...
def dmc_spec2gym_space(spec):
    if isinstance(spec, OrderedDict) or isinstance(spec, dict):
        ...
        if spec.minimum.shape == () and spec.maximum.shape == ():
            new_minimum = np.ones(spec.shape) * spec.minimum
            new_maximum = np.ones(spec.shape) * spec.maximum
            return spaces.Box(
                low=new_minimum, high=new_maximum, shape=spec.shape, dtype=spec.dtype
            )
        return spaces.Box(
